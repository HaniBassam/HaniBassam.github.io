...
export default function Testimonial() {
  const singleTestimonial = data?.testimonial?.[1] || data?.testimonial?.[0]; // Fald tilbage til første, hvis anden ikke findes

  if (!singleTestimonial) {
    return <p>Ingen testimonials tilgængelige</p>; // Tilføj fallback, hvis data ikke findes
  }
    ...
    <section className="testimonial--section" id="testimonial">
      <div className="portfolio--section--container">
        <div className="testimonial--section--card">
          <div className="testimonial--section--card--review">
            {Array.from({ length: singleTestimonial.rating }, (_, index) => (
              <svg
                key={index}
          ...
          <p className="text-md">{singleTestimonial.description}</p>
          <div className="testimonial--section--card--author--detail">
            <img src={singleTestimonial.src} alt="Avatar" />
            <div>
              <p className="text-md testimonial--author--name">
                {singleTestimonial.author_name}
              </p>
              <p className="text-md testimonial--author--designation">
                {singleTestimonial.author_designation}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="portfolio--container-box">
        <div className="portfolio--container">
          <p className="sub--title">You Might Need Me For your next Project </p>
          <h2 className="sections--heading">
            🤖 CV:{" "}
            <a
              href="https://amber-ninnetta-62.tiiny.site"
              target="_blank"
              rel="noopener noreferrer"
              style={{ color: "#006B6A", textDecoration: "none" }}
            >
              https://amber-ninnetta-62.tiiny.site
            </a>
          </h2>
        </div>
      </div>
